...
class DistributeVehiclesWindow(Toplevel):
    ...
    def initializeGUIElements(self):
        ...
        for idx, vehicle in enumerate(self.vehicles):
            self.vehiclesString += "Index " + str(idx) + " represents the vehicle with the id #" + str(
                vehicle.getId()) + ".\n"
    ...
    def executeDistribution(self):
        ...
        try:
            ...
            messagebox.showerror("Population Size Error", "Please check your input. Population size must be a number.",
                                 parent=self)
            return
        ...
        try:
            ...
            messagebox.showerror("Generation Error", "Please check your input. Number of generations must be a number.",
                                 parent=self)
            return
        ...
        self.distributionAlgorithm = GeneticAlgorithm(self.queue,
                                                      args=[self.populationSize, self.generations, self.lanes,
                                                            self.vehicles, self.gaText]).start()
        self.executing = True
...
class ChangeVehiclesWindow(Toplevel):
    ...
    def finish(self):
        ...
        if len(selectedItems) == 0:
            messagebox.showerror("Vehicle Type Not Selected", "At least one vehicle type must be selected.",
                                 parent=self)
            return
...
class AddVehicleWindow(Toplevel):
    ...
    def getVehicleCount(self):
        ...
        try:
            ...
            messagebox.showerror("Vehicle Quantity Error",
                                 "Please check your input. Vehicle quantity must be a number.", parent=self)
            return 0
...
class ConfigWindow(Toplevel):
    ...
    def startConfiguration(self):
        ...
        try:
            ...
            messagebox.showerror("Input Error", "Please check your input. Verify that you wrote only a number.",
                                 parent=self)
            return
    ...
    def startSimulation(self):
        ...
        for key, conf in self.lanesConfiguration.items():
            try:
                ...
                messagebox.showerror("Input Error", "Please check the max capacity of lane " + str(
                    key) + ". Verify that you wrote only a number.", parent=self)
                return

            if maxCapacity <= 0:
                messagebox.showerror("Invalid Max Capacity",
                                     "Max Capacity of lane " + str(key) + " can not be negative or zero.", parent=self)
                return
        ...
        for idx, confList in enumerate(self.configurationLists):
            ...
            if len(selectedItems) == 0:
                messagebox.showerror("Vehicle Type Not Selected",
                                     "At least one vehicle type must be select for lane " + str(idx + 1) + ".",
                                     parent=self)
                return
...
class MainWindow:
    ...
    def loadImages(self):
        ...
        self.toldoImage = PhotoImage(file="toldo.png")
        self.toldoId = []
    ...
    def generateLanes(self):
        ...
        for key, lane in self.lanesConfiguration.items():
            newLane = ServiceLane(key, lane[0], lane[1], self.laneImage, self.toldoImage)
    ...
    def addVehiclesToWaitingLine(self, vehicleType, vehicleCount):
        for x in range(0, vehicleCount):
            ...
            imageId = self.waitingCarsCanvas.create_image(100, 60 * (waitingVehiclesLength - 1) + 25,
                                                          image=self.vehiclesImages[newVehicle.getType()])
            textId = self.waitingCarsCanvas.create_text(100, 60 * (waitingVehiclesLength - 1) + 25,
                                                        fill=self.vehicleIdColor,
    ...
    def updateWaitingLaneImages(self):
        ...
        for idx, vehicle in enumerate(self.waitingVehicles):
            imageId = self.waitingCarsCanvas.create_image(100, 60 * idx + 25,
                                                          image=self.vehiclesImages[vehicle.getType()])
        ...
        for idx, lane in enumerate(self.lanes):
            ...
            if lane.currentVehicle == None:
                ...
                for waitingVehicle in lane.getWaitingVehicles():
                    ...
                    vehicleImageId = self.canvas.create_image(625 + vehicleWidth / 2 + xOff, lane.y + 50,
                                                              image=self.vehiclesImages[waitingVehicle.getType()])
                    vehicleTextId = self.canvas.create_text(625 + vehicleWidth / 2 + xOff, lane.y + 50,
                                                            fill=self.vehicleIdColor,
                                                            font=self.boldFont, text=str(waitingVehicle.getId()))
                    vehicleAttTimeTextId = self.canvas.create_text(625 + vehicleWidth / 2 + xOff, lane.y + 10,
                                                                   fill="black",
                                                                   font=self.boldFont,
                                                                   text=str(waitingVehicle.getRemainingAttentionTime()))
            ...
            # Y hay un carro siendo atendido en el momento de distribuir
                ...
                for waitingVehicle in lane.getWaitingVehicles():
                    ...
                    if waitingVehicle.getImageId() == None:
                        vehicleImageId = self.canvas.create_image(625 + vehicleWidth / 2 + xOff, lane.y + 50,
                                                                  image=self.vehiclesImages[vehicleType])
                        vehicleTextId = self.canvas.create_text(625 + vehicleWidth / 2 + xOff, lane.y + 50,
                                                                fill=self.vehicleIdColor,
                                                                font=self.boldFont, text=str(waitingVehicle.getId()))
                        vehicleAttTimeTextId = self.canvas.create_text(625 + vehicleWidth / 2 + xOff, lane.y + 10,
                                                                       fill="black",
                                                                       font=self.boldFont, text=str(
                                waitingVehicle.getRemainingAttentionTime()))
                        waitingVehicle.setImageAndTextId(vehicleImageId, vehicleTextId, vehicleAttTimeTextId)
                    xOff += vehicleWidth
    ...
    def simulate(self):
        for idx, lane in enumerate(self.lanes):
            ...
            if currentVehicle != None:
                for idx, toldo in enumerate(lane.toldoId):
                    self.canvas.tag_raise(toldo)
    ...
    def drawNewWaitingCar(self, lane):
        ...
        vehicleAttTimeTextId = self.canvas.create_text(xPos, lane.y + 10, fill="black",
                                                       font=self.boldFont,
                                                       text=str(newestVehicle.getRemainingAttentionTime()))
    ...
    def pauseLanes(self):
        ...
        try:
            ...
            messagebox.showerror("Pause Time Input Error",
                                 "Please check your pause time. Verify that you wrote only a number.")
            return
        ...
        if len(pausedLanes) != 0:
            messagebox.showinfo("Paused Lanes", "Lane(s) " + " ".join(
                pausedLanes) + " was/were already paused. You must wait before trying to pause a lane again.")
...
# my_gui.startSimulation()
# waitingVehicles = []
# my_gui.addLanesOptions()
# vehicleTypes = ["Motorcycle > 5 Years", "Motorcycle <= 5 Years", "Vehicles > 5 Years",
#                      "Vehicles <= 5 Years", "Buses", "Two-Axle Truck", "Five-Axle Truck"]
# attentionTimes = [30, 20, 60, 40, 80, 100, 120]
#
# waitingVehicles.append(Vehicle(vehicleTypes[0], attentionTimes[0], 0))
# my_gui.distributeWindow = DistributeVehiclesWindow(5, waitingVehicles, distCount=0, receiveDistribution=0)
# my_gui.startSimulation()
